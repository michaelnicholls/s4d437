...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD issue_message.
    ...
    DATA lo_msg TYPE REF TO cm_d437e_travel.

    CREATE OBJECT lo_msg
      EXPORTING
        textid   = cm_d437e_travel=>cm_d437e_travel
        severity = cm_d437e_travel=>if_abap_behv_message~severity-success.

    APPEND lo_msg TO reported-%other.

* Alternative coding with use of expression NEW
*-----------------------------------------------*
*    APPEND NEW cm_d437e_travel( textid = cm_d437e_travel=>cm_d437e_travel
*                                severity = cm_d437e_travel=>if_abap_behv_message~severity-success )
*       TO reported-%other.
...
*                     ( NEW cm_d437e_travel( textid = cm_d437e_travel=>cm_d437e_travel
*                                            severity = cm_d437e_travel=>if_abap_behv_message~severity-success
*                                          )
*                     )
*                   ).
  ...
  METHOD set_to_cancelled.
    ...
    DATA lt_read_in TYPE TABLE FOR READ IMPORT ZMN_i_travel.
    DATA ls_read_in LIKE LINE OF lt_read_in.

    DATA lt_travel TYPE TABLE FOR READ RESULT ZMN_i_travel.

    DATA lt_update TYPE TABLE FOR UPDATE ZMN_i_travel.
    DATA ls_update LIKE LINE OF lt_update.

    DATA ls_failed TYPE RESPONSE FOR FAILED ZMN_i_travel.
    ...
    LOOP AT lt_travel ASSIGNING <ls_travel>.
      ...
      IF <ls_travel>-status = 'C'. "already cancelled
        CREATE OBJECT lo_msg
          EXPORTING
            textid   = cm_d437e_travel=>already_cancelled
...
* Update Status of travel not yet cancelled
        ...
        MODIFY ENTITY IN LOCAL MODE ZMN_i_travel
...
* Success message
          CREATE OBJECT lo_msg
            EXPORTING
              textid   = cm_d437e_travel=>cancel_success
...
*
  ...
  METHOD determinesemantickey.
    ...
    DATA lt_travel_upd TYPE TABLE FOR UPDATE ZMN_i_travel.
...
***********************************************************************
*
*   READ ENTITIES OF  ZMN_i_travel IN LOCAL MODE
*          ENTITY item
